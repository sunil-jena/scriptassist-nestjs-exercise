...
export class TasksController {
  ...
  async findAll(
    ...
    // };
    const statusEnum = status ? TaskStatus[status as keyof typeof TaskStatus] : undefined;
    const priorityEnum = priority ? TaskPriority[priority as keyof typeof TaskPriority] : undefined;
    return this.tasksService.findAll(statusEnum, priorityEnum, page, limit);
  }
  ...
  async batchProcess(@Body() operations: { tasks: string[], action: string }) {
    ...
    switch (action) {
      ...
      case 'delete':
        return this.tasksService.batchRemove(taskIds);
